...
class GameTree:
    ...
    Instance Attributes:
      ...
      - white_win_probability: which represents the probability
    ...
    Representation Invariants:
        ...
        - self.white_win_probability >= 0 and self.white_win_probability <=1
    """
    move: str
    is_white_move: bool
    white_win_probability: float = 0.0
    ...
    def insert_move_sequence(self, moves: list[str], white_win_probability: float = 0.0) -> None:
        ...
        if moves == []:
            ...
            def check_helper(add: list[str], curr: GameTree, w: float) -> None:
                """a recursive helper method that calls
                `list.pop` on the list of moves."""
                if add == []:
                    return
                else:
                    # pop out the latest move and assign color
                    move_1 = add.pop()
                    move_color = not curr.is_white_move

                    # check if move exists in current subset and update white_win_probability
                    if curr.find_subtree_by_move(move_1) is None:
                        curr.add_subtree(GameTree(move_1, move_color, w))
                    else:
                        traverse_list.append(curr.find_subtree_by_move(move_1))
                    check_helper(add, curr.find_subtree_by_move(move_1), w)

            temp_list = moves[::-1]
            check_helper(temp_list, self, white_win_probability)
            for update in traverse_list[::-1]:
                update._update_white_win_probability()
    ...
    def _update_white_win_probability(self) -> None:
        ...
        if self._subtrees != [] and self.is_white_move is True:
            # list of all subtrees win probabilities
            lst = [subtree.white_win_probability for subtree in self._subtrees]
            max(lst)
            # mutate win
            self.white_win_probability = max(lst)

        # condition 3 check:
        if self._subtrees != [] and self.is_white_move is False:
            # list of all subtrees win probabilities
            lst = [subtree.white_win_probability for subtree in self._subtrees]
            # find avg
            avg = sum(lst) / len(lst)
            ...
            # mutate win
            self.white_win_probability = avg
